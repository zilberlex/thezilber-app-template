...
export function createChangeElementTextContentCommand(target: Element, replacedText: string): Command<boolean> {
	let orgText = '';
...
export function createChangeElementTextContentTemporaryCommand(
	...
	return wrapAutoResetCommand(createChangeElementTextContentCommand(target, replacedText), durationMs);
}
